...
contract L1Block is ISemver, IGasToken {
    ...
    /// @notice Sets the gas paying token information. Can only be called by the depositor account.
    function setGasPayingToken(address _token, uint8 _decimals, bytes32 _name, bytes32 _symbol) external {
        require(msg.sender == DEPOSITOR_ACCOUNT(), "L1Block: only the depositor account can set gas token");
